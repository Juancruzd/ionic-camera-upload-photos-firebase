...
export class AuthPageloginGuard implements CanActivate {
  ...
  canActivate(
    ...
    state: RouterStateSnapshot): Observable<boolean | UrlTree> | Promise<boolean | UrlTree> | boolean | UrlTree {
      ///si el usuario esta logeado lo mandamos a alas tabs ya que no es necesario que este se vuelva a  iniciar sesion,
      ///si no lo esta lo dejamos en la actual pagina.
      return this.ofAuth.authState.pipe(map( auth =>{ 
        if (isNullOrUndefined(auth)) {
          ////no esta logueado se queda en la pagina para que inicie sesion 
          return true;
       }
       else{
